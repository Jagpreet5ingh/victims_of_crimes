...
This dataset includes all identified victims of crimes against the person, including, but not limited to, those that may have been deemed unfounded after investigation, those that may have occurred outside the City of Toronto limits, or have no verified location[@opendatatoronto]

The data has been aggregated by reported year, geographic police division, offence subtype, sex, and age group [@opendatatoronto].

## Limitations

-   Age calculated from date of occurrence [@opendatatoronto]

-   Filtered by reported year [@opendatatoronto]

-   Victims only for CCJS Level 1 = 'Crimes Against the Person' and where the victim name is not null [@opendatatoronto]

## **Data Quality**

As of 23 January, 2024, the dataset was last refreshed on 14 January, 2024 with an overall data quality score of 99% and with a gold rating [@opendatatoronto].

The dataset has a score of 100% for freshness which means the data set is up-to-date and refreshed on schedule [@opendatatoronto].

The dataset has a score of 100% for metadata and completeness which means the data is well described and complete [@opendatatoronto].
...
library("tidyverse")
setwd("/cloud/project") # to ensure directory is not reset to a different path
# Read data
...
ggplot(total_counts, aes(x = as.factor(REPORT_YEAR), y = COUNT_)) +   
  ...
  labs(caption = "Total count of crimes against the person per year (2014-2022)",      
       x = "Year",        
...
ggplot(subtype_counts, aes(x = as.factor(REPORT_YEAR), y = COUNT_, fill = SUBTYPE)) +
  ...
  labs(caption = "Total count of crimes against the person per year by type of crime (2014-2022)",
       x = "Year",
       y = "Total Number of Crimes") +
...
ggplot(sex_counts, aes(x = as.factor(REPORT_YEAR), y = COUNT_, color = SEX)) +
  ...
  labs(caption = "Total count of crimes against the person of each sex per year",
       x = "Year",
       y = "Total Number of Crimes") +
...
ggplot(age_counts, aes(x = as.factor(REPORT_YEAR), y = COUNT_, color = AGE_GROUP, group = AGE_GROUP)) +
  ...
  geom_point(size = 3, alpha = 0.7) +
  labs(caption = "Total count of crimes against the person of each age group per year",
       x = "Year",
       y = "Total Number of Crimes") +
